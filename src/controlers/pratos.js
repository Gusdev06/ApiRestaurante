...
export async function insertprato(req, res) {  //Isso exporta a função insertprato como uma função assíncrona. Isso significa que a função pode conter operações assíncronas e retornar uma promessa.
    ...
    openDb().then(db =>{  //Aqui, a função openDb é chamada para abrir a conexão com o banco de dados. A função then é usada para lidar com a promessa retornada pela função openDb assim que a conexão for estabelecida com sucesso.
        db.run('INSERT INTO pratos (item, descricao, preco, img) VALUES (?, ?, ?, ?)', //Dentro do bloco then, a função run é chamada no objeto db, que representa a conexão com o banco de dados. Isso indica que você está executando uma operação SQL na tabela "prato". A consulta 
         [pratos.item, pratos.descricao, pratos.preco, pratos.img])   // insere um novo registro na tabela "prato" com três colunas: "semana", "prato" e "price". Os valores são inseridos na consulta usando a notação ?, que é substituída pelos valores reais durante a execução. Os valores reais são fornecidos como um array [prato.semana, prato.prato, prato.price].
    });
...
export async function Updateprato(req, res) {  
    ...
    openDb().then(db =>{  
        ...
        [pratos.item, pratos.descricao, pratos.preco,  pratos.img, pratos.id]) 
    });
...
export async function selectprato(req, res) {  
    let id = req.body.id;
    openDb().then(db =>{  
        db.get('SELECT * FROM pratos WHERE id=?',[id] )
        .then(pratos =>res.json(pratos));
    });
  }
